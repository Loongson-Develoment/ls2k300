...
#define INJECT_PUSH_STATUS_PRINT_MS 1000LL  /* 主推药电机推药状态打印周期，单位 ms */
                                    ...
                                    ArmRuntime *runtime)
{
    ...
    while (running && !shutdown_requested && injection_running) {
        ...
        if (now_ms - start_ms > INJECT_PUSH_WAIT_TIMEOUT_MS) {
            APP_LOG_WARN("inject push timeout: pos=%.4fml, target=%.4fml, "
                         "target_speed=%.4fml/s, measured_speed=%.4fml/s",
                         inject_control->Motor_position_ml(),
                         inject_control->Motor_target_position_ml(),
                         inject_control->Motor_target_speed_mlps(),
                         inject_control->Motor_filtered_speed_mlps());
            inject_control->Motor_stop();
            return false;
        }
        ...
        if (last_print_ms <= 0 ||
            now_ms - last_print_ms >= INJECT_PUSH_STATUS_PRINT_MS) {
            APP_LOG_INFO("inject push status: pos=%.4fml, target=%.4fml, "
                         "target_speed=%.4fml/s, measured_speed=%.4fml/s",
                         inject_control->Motor_position_ml(),
                         inject_control->Motor_target_position_ml(),
                         inject_control->Motor_target_speed_mlps(),
        ...
        if (inject_control->Is_arrived()) {
            inject_control->Motor_stop();
            APP_LOG_INFO("inject push arrived: pos=%.4fml, target=%.4fml",
                         inject_control->Motor_position_ml(),
                         inject_control->Motor_target_position_ml());
            return true;
        }
