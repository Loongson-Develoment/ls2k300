...
void usart_SendCmd(ls2k0300_uart_t *uart, uint8_t *cmd, uint8_t len)
{
  for (uint8_t i = 0; i < len; ++i) {
    ls2k0300_uart_write(uart, &cmd[i], 1);
  }
}
/**
  * @brief    将当前位置清零
  ...
  */
void ZDT_X42_V2_Reset_CurPos_To_Zero(ls2k0300_uart_t *uart, uint8_t addr)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Reset_Clog_Pro(ls2k0300_uart_t *uart, uint8_t addr)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Read_Sys_Params(ls2k0300_uart_t *uart, uint8_t addr, SysParams_t s)
{
  uint8_t cmd[16] = {0};
  ...
  if(s >= S_Conf)
  {
    cmd[3] = 0x6B; usart_SendCmd(uart, cmd, 4);
  }
  else
  {
    cmd[2] = 0x6B; usart_SendCmd(uart, cmd, 3);
  }
}
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Modify_Ctrl_Mode(ls2k0300_uart_t *uart, uint8_t addr, bool svF, uint8_t ctrl_mode)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_En_Control(ls2k0300_uart_t *uart, uint8_t addr, bool state, uint8_t snF)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Torque_Control(ls2k0300_uart_t *uart, uint8_t addr, uint8_t sign, uint16_t t_ramp, uint16_t torque, uint8_t snF)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Velocity_Control(ls2k0300_uart_t *uart, uint8_t addr, uint8_t dir, uint16_t v_ramp, float velocity, uint8_t snF)
{
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Bypass_Position_LV_Control(ls2k0300_uart_t *uart, uint8_t addr, uint8_t dir, float velocity, float position, uint8_t raf, uint8_t snF)
{
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Traj_Position_Control(ls2k0300_uart_t *uart, uint8_t addr, uint8_t dir, uint16_t acc, uint16_t dec, float velocity, float position, uint8_t raf, uint8_t snF)
{
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Stop_Now(ls2k0300_uart_t *uart, uint8_t addr, uint8_t snF)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Synchronous_motion(ls2k0300_uart_t *uart, uint8_t addr)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Origin_Set_O(ls2k0300_uart_t *uart, uint8_t addr, bool svF)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Origin_Modify_Params(ls2k0300_uart_t *uart, uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  uint8_t cmd[32] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Origin_Trigger_Return(ls2k0300_uart_t *uart, uint8_t addr, uint8_t o_mode, bool snF)
{
  uint8_t cmd[16] = {0};
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void ZDT_X42_V2_Origin_Interrupt(ls2k0300_uart_t *uart, uint8_t addr)
{
  uint8_t cmd[16] = {0};
...
//   * @retval   无
//   */
// void ZDT_X42_V2_Receive_Data(ls2k0300_uart_t *uart, uint8_t *rxCmd, uint8_t *rxCount)
// {
