...
/* Legacy implementation is kept above for reference.
 * The replacement below avoids holding uart->mtx across blocking reads,
 * so another thread can still send while one thread is waiting for data.
 */

static int ls2k0300_uart_is_invalid(const ls2k0300_uart_t *uart)
{
    return (uart == NULL || uart->initialized == 0 || uart->fd < 0);
}
 ...
 ********************************************************************************/
int ls2k0300_uart_init(ls2k0300_uart_t *uart, const char *path, speed_t baud,
                       ls_uart_stop_bits_t stop, ls_uart_data_bits_t data,
                       ls_uart_parity_t parity, ls_uart_mode_t mode)
{
    if (uart == NULL || path == NULL) {
        return -1;
    }
...
int ls2k0300_uart_block_init(ls2k0300_uart_t *uart, const char *path, speed_t baud,
                             ls_uart_stop_bits_t stop, ls_uart_data_bits_t data,
                             ls_uart_parity_t parity, uint32_t char_num)
{
    int ret = ls2k0300_uart_init(uart, path, baud, stop, data, parity, LS_UART_MODE_BLOCKING);
...
ssize_t ls2k0300_uart_write(ls2k0300_uart_t *uart, const uint8_t *buf, size_t len)
{
    ...
    if (ls2k0300_uart_is_invalid(uart) || buf == NULL || len == 0U) {
        return -1;
    }
...
/********************************************************************************
 * @brief   UART 接收函数.
 * @note    读取路径不持有 uart->mtx，避免阻塞接收把发送也一起卡住.
 ********************************************************************************/
ssize_t ls2k0300_uart_read(ls2k0300_uart_t *uart, uint8_t *buf, size_t len)
{
    if (ls2k0300_uart_is_invalid(uart) || buf == NULL || len == 0U) {
                               ...
                               int inter_timeout_ms)
{
    ...
    if (ls2k0300_uart_is_invalid(uart) || buf == NULL || len == 0U || inter_timeout_ms < 0) {
        return -1;
    }
    ...
    while (total < len) {
        ...
        n = read(uart->fd, buf + total, len - total);
...
int ls2k0300_uart_flush(ls2k0300_uart_t *uart)
{
    ...
    if (ls2k0300_uart_is_invalid(uart)) {
        return -1;
    }
